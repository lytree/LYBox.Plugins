...
namespace LYBox.Plugin.Downloader.WebServer;
...
public sealed class WebConsoleServer : IDisposable
{
    ...
    public WebConsoleServer(DownloadCoordinator coord, DouyinSettingsStore settings)
    {
        _coord = coord;
        _settings = settings;
    ...
    private async Task HandleAsync(HttpListenerContext ctx)
    {
        try
        {
            ...
            if (method == "POST" && path == "/api/v1/settings")
            {
                ...
                try
                {
                    var patch = JsonSerializer.Deserialize<DouyinSettings>(body, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                    if (patch != null)
                    {
                        foreach (var p in typeof(DouyinSettings).GetProperties())
