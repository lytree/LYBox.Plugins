...
/// <summary>
/// 抖音下载总调度：消费队列 → 选择策略 → 调用 RateLimiter/Retry → 更新 JobRow。
/// 与 <c>Services.DownloadOrchestrator</c>（点播/直播单任务编排）区分，故保留 Douyin 前缀。
/// </summary>
public sealed class DouyinDownloadOrchestrator : IHostedServiceLite
{
    ...
    public DouyinDownloadOrchestrator(
        ...
        DouyinSettingsStore settings, LiveSessionRegistry liveRegistry)
    {
        _queue = queue;
        _factory = factory;
        _api = api;
    ...
    public async Task ExecuteOneAsync(DownloadJobRow row, CancellationToken ct)
    {
        ...
        var ctx = new JobContext(
            ...
            Log: msg => Utils.Logger.Info($"[Job {row.JobId}] {msg}"));
