...
using LYBox.Plugin.Downloader.WebServer;
...
{
    private readonly DouyinSettingsStore? _store;
    ...
    {
        _store = ServiceLocator.TryGetService<DouyinSettingsStore>(out var s) ? s : null;
        _web = ServiceLocator.TryGetService<WebConsoleServer>(out var w) ? w : null;
        if (_store is null)
        {
            StatusText = "DouyinSettingsStore 服务解析失败 — 请查看应用日志";
            return;
        }
    ...
    private void Save()
    {
        if (_store is null) { StatusText = "DouyinSettingsStore 未就绪,无法保存"; return; }
        var cur = _store.Current;
